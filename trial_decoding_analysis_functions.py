"""Contains helper functions for the decoding analysis. These functions include data loading, preprocessing, epoching,
and decoding steps. They are designed to be modular and reusable across different subjects and sessions in the dataset."""
...
def decoding(epochs_normal, epochs_conflict, show_plots=False):
    """Performs time-resolved decoding of the normal vs. conflict condition using a logistic regression classifier and
    plots the decoding scores (AUC) over time if required.
    
    Args:
        epochs_normal: The epoched data for the normal condition.
        epochs_conflict: The epoched data for the conflict condition.
        show_plots: Whether to plot the decoding scores over time for visual inspection.
    
    Returns:
        The decoding scores (AUC) over time.
    """
    
    epochs_all = mne.concatenate_epochs([epochs_normal, epochs_conflict])
    cond_code = np.concatenate([np.zeros(len(epochs_normal), dtype=int), np.ones(len(epochs_conflict), dtype=int)])
    
    X = epochs_all.get_data()[:, 0:1, :]  # Get data for FCz channel
    y = cond_code # 0 = match, 1 = mismatch
    
    clf = LogisticRegression(max_iter=500)
    time_decod = SlidingEstimator(LinearModel(clf),
        scoring="roc_auc",
        n_jobs=1)
    ...
    if show_plots:
        plt.figure(figsize=(10, 5))
        plt.plot(epochs_all.times, scores_mean, color="blue")
