"""Contains helper functions for loading, preprocessing, and analyzing the EEG data for the ERP analysis."""
...
def make_erp(data, condition="conflict", show_plots=False):
    ...
    # Apply final band-pass filter on the epoched data -> bad as it shifts the data
    #epochs = epochs.copy().filter(l_freq=0.5, h_freq=45.0)
...
def make_erp_plot(normal_data, conflict_data, diff_data):
    """Plots the ERPs for the normal and conflict conditions for the grand average data.
    
    Args:
        normal_data: The grand average ERP data for the normal condition.
        conflict_data: The grand average ERP data for the conflict condition.
        diff_data: The grand average ERP data for the difference condition.
    """
    ...
    figs = mne.viz.plot_compare_evokeds(
        ...
        time_unit="ms",
        colors=colors,
        title="A.) Match Trials (FCz)",
        ylim=dict(eeg=[-4, 12])
    )
    fig = figs[0]
    fig.set_size_inches(6, 4)
    fig.savefig("match.pdf", format="pdf")
    ...
    figs = mne.viz.plot_compare_evokeds(
        ...
        time_unit="ms",
        colors=colors,
        title="B.) Mismatch Trials (FCz)",
        ylim=dict(eeg=[-4, 7])
    )
    fig = figs[0]
    fig.set_size_inches(6, 4)
    fig.savefig("mismatch.pdf", format="pdf")
    ...
    figs = mne.viz.plot_compare_evokeds(
        ...
        time_unit="ms",
        colors=colors,
        title="C.) Difference Mismatch-Match Trials (FCz)",
        ylim=dict(eeg=[-7, 6])
    )
    fig = figs[0]
    fig.set_size_inches(6, 4)
    fig.savefig("diff.pdf", format="pdf")
